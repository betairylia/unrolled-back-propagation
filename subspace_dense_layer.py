...
class MaskedDenseLayer(Layer):
    ...
    def __init__(
        ...
        with tf.variable_scope(name):
            ...
            if mask is not None:
                masked_W = tf.multiply(W, mask)
                self.outputs = tf.matmul(self.inputs, masked_W)
            else:
...
def SubSpaceWrapper(
    ...
    mask = np.zeros((n_in, n_units))
    subLen = n_units // subDiv
    
    for s in range(subDiv):
        
        subStart = s - subConnect
        subEnd = s + subConnect + 1

        subStart = max(subStart, 0)
        subEnd = min(subEnd, subDiv)

        mask[s * subLen : (s + 1) * subLen, (subStart * subLen) : (subEnd * subLen)] = 1
