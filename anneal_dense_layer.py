...
class AnnealDenseLayer(Layer):
    ...
    def __init__(
        ...
        super(AnnealDenseLayer, self
             ).__init__(prev_layer=prev_layer, act=act, W_init_args=W_init_args, b_init_args=b_init_args, name=name)

        logging.info(
            "AnnealDenseLayer  %s: %d %s" %
        ...
        with tf.variable_scope(name):
            ...
            if is_train == True:
                noised_W = tf.add(tf.random.normal(shape = (n_in, n_units), mean = 0.0, stddev = sigma), W, name = 'noised_W')
            else:
                noised_W = W
            
            self.outputs = tf.matmul(self.inputs, noised_W)
            ...
            if b_init is not None:
                ...
                if is_train == True:
                    noised_b = tf.add(tf.random.normal(shape = b.shape, mean = 0.0, stddev = sigma), b, name = 'noised_b')
                else:
                    noised_b = b

                self.outputs = tf.nn.bias_add(self.outputs, noised_b, name='bias_add')
